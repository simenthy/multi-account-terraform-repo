# Root outputs for the consolidated account.
#
# The nine networking outputs are declared exactly as the former networking root
# declared them. The merged state carries those output values across, so they
# show NO diff on the first plan — only the compute outputs appear as additions.
# (`terraform state mv` moves resources but not root outputs, so some output
# churn on the first plan is expected and touches nothing in AWS.)
#
# NOTE: these are no longer consumed cross-domain — everything that needed them
# now reads module outputs directly in main.tf. They are kept for human
# inspection (`terraform output subnet_ids`) and for any external tooling.
